...
void __stdcall dump()
{
	Minecraft Minecraft = cheat_main::get().mapper()->findClass(212, 102);
	
	jclass BlockPos = cheat_main::get().mapper()->findClass(30, 9);
	
	FontRenderer fontRenderer = cheat_main::get().mapper()->findClass(54, 38); // Minecraft.fields[32].sig;

	jclass Entity = cheat_main::get().mapper()->findClass(237, 75, true,
		search_options_t
	{
	{},
	{ {"I", 16} }
	...
	jclass WorldClient = cheat_main::get().mapper()->findClass(37, 7, false, 
		search_options_t
	{
	{},{  {"Ljava/util/Set;", 2}}
	});
	...
	jclass ItemStack = cheat_main::get().mapper()->findClass(101, 11, true, search_options_t({
	{}, { {"I", 3}, {"Z", 2}}
		}));

	
	jclass EntityPlayerSP = cheat_main::get().mapper()->findClass(87, 24, true);
	...
	//cheat_main::get( ).mapper( )->findClass( 39, 10 );
	PlayerControllerMP PlayerController = cheat_main::get().mapper()->findClass(39, 10, true, search_options_t(
	{
		{ }, { {"F", 2}, {"I", 2}, {"Z", 1}}
	}));
	
	jclass Potion = cheat_main::get().mapper()->findClass(29, 42, true);
	mc_class Block = cheat_main::get().mapper()->findClass(138, 41);
	...
	NetHandlerPlayClient NetHandlerPlayClient = cheat_main::get().mapper()->findClass(102, 11, true, search_options_t(
		{
			{}, { { "Z", 2}, { "I", 1}}
		}));

	
	jclass GameSettings = cheat_main::get().mapper()->findClass(138, 41);
	...
	printf("\n\nMethods:\n");
	/*
	for (const method_t method : PlayerController.methods)
	{
	...
	for (const method_t method : EntityLivingBase.methods)
	{
		std::string name = method.name;
		std::string signature = method.sig;
	...
	for (const method_t method : NetHandlerPlayClient.methods)
	{
		std::string name = method.name;
		std::string signature = method.sig;
	...
	for (const auto& method : RenderManager.methods)
	{
		std::string name = method.name;
		std::string signature = method.sig;
	...
	}
	{
		...
		for (auto& method : IBlockAccess.methods)
		{
			...
			const auto vstrings = util::split(parameters, ';');
		...
		IBlockState_name.erase(0, 1);
		IBlockState_name.erase(IBlockState_name.size() - 1);

		mc_class IBlockState = cheat_main::get().mapper()->findClass(IBlockState_name.c_str());


		for(auto& method : IBlockState.methods)
		{
			std::string name = method.name;
			std::string signature = method.sig;

			if (signature.find(jvmutil::getClassName(Block.clazz, true)) != std::string::npos)
			{
				std::cout << "getBlock: " << method.name << " : " << method.sig << std::endl;
			}
		}
	}
...
int __stdcall entrypoint()
{
	...
	if( cheat_main::get().init() == TRUE)
	{
		...
		{
			//jvmutil::print_class_info( cheat_main::get( ).mapper( )->findClass( "net.minecraft.v1_8.seahppsessssppphapsaheppa" ) );
			//jvmutil::print_class_info( cheat_main::get( ).mapper( )->findClass( "net.minecraft.v1_8.pshashhesashphshassasheaa" ) );
			dump();

			//jvmtiError err;
			//mc_class klass = cheat_main::get().mapper()->findClass(71, 12);
			...
			//
			//if (klass.clazz != nullptr) 
			//{
			//	//err =  cheat_main::get().jvmti->GetConstantPool(klass, &cp_cnt, &cp_bytes_cnt, &cp_bytes);

			//	auto mid = cheat_main::get().env->GetMethodID(klass.clazz, klass.methods[0].name, klass.methods[0].sig);
			//	err = cheat_main::get().jvmti->GetLocalVariableTable(mid, &local_entry_count_ptr, &table_ptr);

			//	if (err != JVMTI_ERROR_NONE) {
			//		printf("=> %s\n", table_ptr->name);
			//		//if (table_ptr != nullptr) 
			//		//{
			//		//	for (int i = 0; table_ptr->length; i++) {
			//		//		std::cout << table_ptr[i].name << std::endl;
			//		//	}
			//		//}
			//	}
			//}
