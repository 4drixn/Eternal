...
bool ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        ItemAdd(bb, id);
        bool itemHovered, pressed;
        ImGui::ButtonBehavior(bb, id, &itemHovered, &pressed);
        return pressed;
    }
    else
    {
...
bool ImGui::tab(const char* label, bool selected)
{
    ...
    static std::map<ImGuiID, ImRect> animation_k;
    ...
    static ImVec2 menu_pos = GetWindowPos();

    if (selected)
    {
        static ImRect bb_static(pos, pos + size);

        if (menu_pos.x == GetWindowPos().x)
        {
            bb_static.Min.x = ImLerp(bb_static.Min.x, bb.Min.x, ImGui::GetIO().DeltaTime * 9.f);
            bb_static.Max.x = ImLerp(bb_static.Max.x, bb.Max.x, ImGui::GetIO().DeltaTime * 9.f);

            bb_static.Min.y = bb.Min.y;
            bb_static.Max.y = bb.Max.y;

        }
        else
        {
            bb_static.Min = bb.Min;
            bb_static.Max = bb.Max;
        ...
        draw->AddRectFilled(ImVec2(bb_static.Min.x, bb_static.Max.y - 2), bb_static.Max, ImColor(112, 146, 190, (int)(255 * ImGui::GetStyle().Alpha)));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), ImGui::GetColorU32(ImGuiCol_WindowBg), style.FrameRounding);
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImGui::GetColorU32(ImVec4(0.08, 0.08, 0.08, 1.f)), style.FrameRounding);


    auto fid = (int)id;

    float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
    float pad = 3.f;//ImMax(1.0f, (float)(int)(check_sz / 6.0f));
    ...
    if (*v || changed) {
       // window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, ImGui::GetColorU32(ImVec4(0.24f, 0.40f, 0.95f, 1.00f)), ImGui::GetColorU32(ImVec4(0.24f, 0.40f, 0.95f, 1.00f)), ImGui::GetColorU32(ImVec4(0.0f, 0.56f, 1.00f, 1.00f)), ImGui::GetColorU32(ImVec4(0.0f, 0.56f, 1.00f, 1.00f))); // Main gradient.
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max , GetColorU32(ImGuiCol_SliderGrab), style.FrameRounding);
    }
    if (changed/* && *v*/)
    ...
    if (hovered && *v)
        window->DrawList->AddRect(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), GetColorU32(ImVec4(0.25, 0.25, 0.25, 1.f)), style.FrameRounding);
    else if (hovered && !*v)
...
//    const ImVec2 label_size = CalcTextSize(label, NULL, true);
//
//    const float square_sz = GetFrameHeight() * 1.05;
...
//
//    const ImVec4 hover_act = ImVec4(0.28, 0.29f, 0.32, 1.f);
//    const ImVec4 hover_dis = ImVec4(0.16f, 0.34f, 0.85f, 1.f);
//
...
//        it_filled->second = math::clamp(it_filled->second - 1.7f * ImGui::GetIO().DeltaTime, 0.5f, 0.9f);
//    else
//        it_filled->second = math::clamp(it_filled->second + (1.7f * ImGui::GetIO().DeltaTime * ((*v) ? 1.f : -1.f)), 0.0f, 0.9f);
//
//    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
//
//    RenderNavHighlight(total_bb, id);
//    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_ChildBg), true, style.FrameRounding);
//
//    window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)));
//   // window->DrawList->AddRect(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), ImGui::GetColorU32(ImVec4(0.04705882352, 0.04705882352, 0.04705882352, 1.f)));
//
//    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(0.24f, 0.52f, 0.88f, it_filled->second)), 2.f);
//
//    if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float arrow_size = (flags & ImGuiComboFlags_NoArrowButton) ? 0.0f : GetFrameHeight() * 1.30;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float expected_w = CalcItemWidth();
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : expected_w;

    const float desired_y = label_size.y + style.FramePadding.y * 2.0f;
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
	...
	// * 25
    it_hover->second = math::clamp(it_hover->second + (1.5 * ImGui::GetIO().DeltaTime * (30.f)), 0.0f, desired_y);

    ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, it_hover->second));
   ...
   // const ImU32 frame_col = GetColorU32(hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    const ImU32 frame_col = GetColorU32(ImVec4(0.15, 0.14, 0.15, 1));
    const ImU32 frame_col2 = GetColorU32(ImVec4(0.12, 0.12, 0.12, 1));
    const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);

    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(0.30f, 0.30f, 0.30f, 1.f)), 0.f);

    RenderNavHighlight(frame_bb, id);
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
          window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, frame_col, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
          //window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, frame_col, frame_col, frame_col2, frame_col2);
    }
    ...
    if (!popup_open)
    {
        RenderArrow(window->DrawList, ImVec2(frame_bb.Max.x - 12 - style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.0f);
    }
    ...
    static std::map<ImGuiID, float> combo_animation;
    auto it_ani = combo_animation.find(id);

    if (it_ani == combo_animation.end())
    {
        combo_animation.insert({ id, 10.f });
        it_ani = combo_animation.find(id);
    }

    //static std::map<ImGuiID, float> combo_animation2;
    //auto it_ani2 = combo_animation2.find(id);

    //if (it_ani2 == combo_animation2.end())
    //{
    //    combo_animation2.insert({ id, 18.f });
    //    it_ani2 = combo_animation2.find(id);
    //}
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
    {
        if (popup_window->WasActive)
        {
           ...
           // it_ani2->second = math::clamp(it_ani2->second + (2 * ImGui::GetIO().DeltaTime * 1.75), 18.f, 30.f);
            // it_ani2->second = math::interpolate(it_ani2->second, 30.f, ImGui::GetIO().DeltaTime * 3);
 

            it_ani->second = ImLerp(it_ani->second, size_expected.y + 4.f, ImGui::GetIO().DeltaTime * 6.f);
          //  it_ani->second = math::clamp(it_ani->second + (2 * ImGui::GetIO().DeltaTime * 34.f), 0.f, size_expected.y + 4.f);

            if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            {
                RenderArrow(window->DrawList, ImVec2(frame_bb.Max.x - 12 - style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Up, 1.0f);
            }

            pos.y += 2;
            SetNextWindowPos(pos);
            SetNextWindowSize(ImVec2(100, it_ani->second));
        }
        else {
            it_ani->second = 0.0f;
            //it_ani2->second= 0.0f;
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (Selectable(item_text, item_selected, 0,  ImVec2(ImGui::GetItemRectSize().x, 12)))
        {
            value_changed = true;
            *current_item = i;
        }
...
//    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, g.Style.FrameRounding);
    ...
    static std::map<ImGuiID, float> slider_animation;
    auto it_slider = slider_animation.find(id);
    if (it_slider == slider_animation.end())
    {
        slider_animation.insert({ id, frame_bb.Min.x });
        it_slider = slider_animation.find(id);
    }

    bool min = false;
    if (value_changed)
    {
        if (it_slider->second > grab_bb.Max.x) {
            min = true;
        }
        else if (it_slider->second < grab_bb.Max.x)
            min = false;
    }

   // if(changed)
    if (global::window_pos_changed == true)
    {
        it_slider->second = grab_bb.Max.x;
    }

  
    //if (it_slider->second != 0.f){

        if (!min)
        {
            it_slider->second = math::clamp(it_slider->second + (3.f * ImGui::GetIO().DeltaTime *  30.f ), 50.f, grab_bb.Max.x);
        }
        else if (min)
        {
            std::cout << "MIN\n";
            it_slider->second = math::clamp(it_slider->second - (50.f * ImGui::GetIO().DeltaTime ), 50.f, grab_bb.Max.x);
        }
    

   // it_slider->second = math::clamp(it_slider->second + (3.f * ImGui::GetIO().DeltaTime * 30.f), 3.0f, grab_bb.Max.x);
    
    ImColor borderColor1 = ImColor(38, 36, 38); // 118, 136, 182
    auto backgroundColor = ImColor(34, 32, 34);
    const ImU32 frame_col2 = GetColorU32(ImVec4(0.15, 0.14, 0.15, 1));

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, frame_col2, 1.7f, 15); // background	
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 1), ImVec2(it_slider->second , grab_bb.Max.y) + ImVec2(1, 1), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 2, 15);
...
bool ImGui::SliderScalarF(const char* label, ImGuiDataType data_type, float* p_data, const float* p_min, const float* p_max, const char* format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const bool hovered = ItemHoverable(frame_bb, id);
    bool held;
    bool pressed = ButtonBehavior(frame_bb, id, nullptr, &held);
    bool temp_input_is_active = TempInputTextIsActive(id);
    bool temp_input_start = false;
    if (!temp_input_is_active)
    {
    ...
    //static std::map<ImGuiID, float> slider_animation2;
    //auto it_slider2 = slider_animation2.find(id);
    //if (it_slider2 == slider_animation2.end())
    //{
    //    slider_animation2.insert({ id, 0 });
    //    it_slider2 = slider_animation2.find(id);
    //}
    ...
    if (it_slider->second > frame_bb.Max.x)
    {
        it_slider->second = grab_bb.Max.x;
    }

    if (it_slider->second < frame_bb.Min.x)
    {
        it_slider->second = grab_bb.Max.x;
    }
    ...
    it_slider->second = ImLerp(it_slider->second, grab_bb.Max.x, ImGui::GetIO().DeltaTime * 6.f);

    const ImU32 frame_col2 = GetColorU32(ImVec4(0.15, 0.14, 0.15, 1));

    //window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 1), ImVec2(it_slider->second, grab_bb.Max.y) + ImVec2(1, 1), GetColorU32(ImGuiCol_SliderGrab), GetColorU32(ImGuiCol_SliderGrab), GetColorU32(ImGuiCol_SliderGrabActive), GetColorU32(ImGuiCol_SliderGrabActive));

 //   window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, frame_col2, 0, 0); // background
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 1), ImVec2(it_slider->second, grab_bb.Max.y) + ImVec2(1, 1), GetColorU32(ImGuiCol_SliderGrab), 3.f); // background

    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(ImVec4(0.14, 0.14, 0.14, 1.f)), 3.f);

    if (hovered)
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(ImVec4(0.25, 0.25, 0.25, 1.f)), 3.f);
    ...
    if (it_slider->second < grab_bb.Max.x)
    {
      //  it_slider->second = math::clamp(it_slider->second + (3.f * ImGui::GetIO().DeltaTime * 40.f), 50.f, grab_bb.Max.x);
        it_slider->second = math::interpolate(it_slider->second, grab_bb.Max.x, io.DeltaTime * 8);
    }
    if (it_slider->second > grab_bb.Max.x)
    {
        if (it_slider->second > grab_bb.Max.x)
        {
            ...
            //it_slider->second -= 40.f * 3.f * io.DeltaTime;//0.5;
    }
   ...
   // it_slider->second = math::clamp(it_slider->second + (3.f * ImGui::GetIO().DeltaTime * 40.f), 3.0f, grab_bb.Max.x);
   ...
   // if (grab_bb.Max.x > grab_bb.Min.x)
    ...
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(((*p_data - *p_min) / (*p_max - *p_min)), 0.5f) );
...
{
    return SliderScalarF(label, ImGuiDataType_Float, v, &v_min, &v_max, format, power);
}
...
{
    return SliderScalarF(label, ImGuiDataType_S32, (float*)v, (float*)&v_min, (float*)&v_max, format, 0);
}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 size(label_size.x, size_arg.y);
    ImVec2 pos = window->DC.CursorPos;
    pos.y += window->DC.CurrLineTextBaseOffset; //1
    ...
    const float spacing_y = style.ItemSpacing.y + 1.5f;
    ...
    if (selected)
    {
        ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_SliderGrab));
