...
// unloading manual mapped module
...
__forceinline BOOL RemoveEntryList(PLIST_ENTRY Entry)
{
...
__forceinline BOOL ClearPEHeader(HINSTANCE hModule)
{
...
__forceinline BOOL WINAPI HideDll(HINSTANCE hInstance)
{
...
inline std::vector<UNLINKED_MODULE> UnlinkedModules;
...
__forceinline void UnlinkModuleFromPEB(HMODULE hModule)
{
...
__forceinline bool RemoveHeader(HINSTANCE hinstDLL)
{
	DWORD ERSize = 0;
	DWORD Protect = 0;
...
__forceinline bool HideThread(HANDLE hThread)
{
	...
	pNtSetInformationThread NtSIT = (pNtSetInformationThread)GetProcAddress(GetModuleHandleA(xorstr_("ntdll.dll")), xorstr_("NtSetInformationThread"));
...
//__forceinline void HideModule2(HANDLE hModule)
//{
//	PIMAGE_DOS_HEADER pDOSHeader = (PIMAGE_DOS_HEADER)hModule;
//	if (pDOSHeader->e_magic == IMAGE_DOS_SIGNATURE)
//	{
